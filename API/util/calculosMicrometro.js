...
function calculoParalelismo(dadosParalelismo, req) {
  ...
  req.valpara = desvPadparalelismo
...
function incertez_medEader (req){
  ...
  const response = {"incertezaPD": parseFloat(incertezaPD.toFixed(5)), "contribuoção_incereteza": parseFloat(contriIncerteza.toFixed(5))}
...
function incertezaUC(req){
  ...
  const somaQuadrados = req.incerteza.reduce((acc, val) => acc + Math.pow(val, 2), 0);
  ...
  const response = {"UC": parseFloat(raiz), "veff": parseFloat(veff), "K=": parseFloat(K_Resposta), "U=": parseFloat(U_Arredondado.toFixed(3))}
